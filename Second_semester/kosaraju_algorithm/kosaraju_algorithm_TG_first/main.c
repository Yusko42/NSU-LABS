...
//Building a transpose graph by reversing an adj list
void trans_list(GRAPH* graph, GRAPH* trans_graph, int vert){
    for (int i = 0; i < vert; i++){
        VERTEX* cur_vrt = graph->adj_list[i];
        while (cur_vrt) {
            if (cur_vrt->vert + 1)
                add_edge(trans_graph, cur_vrt->vert + 1, i + 1);
            cur_vrt = cur_vrt->next;
        }
    }
}
...
void kosaraju(GRAPH* graph){
    ...
    trans_list(graph, trans_graph, verts);
